...
const TARGET_BYTES = 20 * 1024 * 1024; // 20 MB — leaves 5 MB margin under Discord's 25 MB limit
const BYTES_PER_PIXEL = 1.5;           // conservative GIF compression estimate
const MAX_FPS = 8;
const MIN_FPS = 4;
const MAX_SCALE = 320;
const MIN_SCALE = 120;
...
function pickSettings(duration: number, origWidth: number, origHeight: number): { fps: number; scale: number } | null {
    ...
    for (let fps = MAX_FPS; fps >= MIN_FPS; fps -= 2) {
        ...
        const scale = Math.max(MIN_SCALE, Math.min(MAX_SCALE, Math.floor(maxScale / 2) * 2)); // round down to even
...
export async function videoToGif(url: string, ext: string): Promise<GifResult> {
    ...
    try {
        ...
        const { fps, scale } = info
            ? pickSettings(info.duration, info.width, info.height) ?? { fps: MIN_FPS, scale: MIN_SCALE }
